...
- ⏱️ **Time Tracking** — Pencatatan waktu pengerjaan (menit)
...
- 📋 **Project Detail Page** — Header, progress, anggota tim, milestones, tasks terbaru
...
- 📄 **Milestone Detail Page** — Info lengkap, stats, daftar task dalam milestone
...
- 🌐 **i18n (ID/EN)** — Multi-language tanpa reload, persistent preference, fully translated semua halaman
...
- 🖱️ **Lenis Smooth Scroll** — Smooth scrolling tanpa memory leak
- 📱 **Responsive** — Mobile bottom nav ↔ Desktop sidebar

### Data & Security
- 💾 **LocalStorage Persistence** — Semua data tersimpan lokal, validasi corrupt data
- 🛡️ **Backward Compatible Migration** — Data lama otomatis dimigrasi saat field baru ditambahkan (memberIds, clientId, phone, address, notes, updatedAt)
...
- **Project → User**: Many-to-Many (via memberIds array)
...
│ ├── task/ # TaskCard, TaskList, AddTaskModal, EditTaskModal, DeleteTaskModal, TaskDetailModal
│ └── ui/ # Button, Badge, Modal, ProgressBar, EmptyState, ConfirmDialog
├── constants/ # Categories, Priorities, Badge styles
├── context/
│ ├── AuthContext.tsx # Auth, RBAC permissions, user override system
│ ├── TaskContext.tsx # Task CRUD, checklist, comments, approval
│ ├── ProjectContext.tsx # Project CRUD, membership, client relation
...
│ ├── Projects/ # Project list + ProjectDetail
...
│ ├── Settings/ # Language, theme, danger zone
│ └── Login/ # Mock login (select user)
