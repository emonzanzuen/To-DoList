...
export default function ProjectsPage() {
  ...
  const [form, setForm] = useState({ name: '', description: '', status: 'active' as Project['status'], dueDate: '' });
  ...
  const projectStats = useMemo(() => {
    return searchedProjects.map((project) => {
      ...
      // Gunakan project dueDate jika ada, fallback ke nearest task deadline
      const upcomingTasks = projectTasks
        .filter((t) => t.status !== 'completed' && t.dueDate)
        .sort((a, b) => (a.dueDate ?? 'z').localeCompare(b.dueDate ?? 'z'));
      const nearestDeadline = project.dueDate || (upcomingTasks.length > 0 ? upcomingTasks[0].dueDate : null);
      ...
      {showForm && (
        <div data-animate className="space-y-4 rounded-2xl border border-line bg-surface p-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-muted">{t('project.deadline')}</label>
              <input
                type="date"
                className={inputClass}
                value={form.dueDate}
        ...
        {visibleProjects.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {projectStats.map(({ project: p, total, completed, inProgress, overdue, pct, projectMilestones, nearestDeadline }) => {
              ...
              const isProjectOverdue = p.dueDate && p.status !== 'completed'
                ? new Date(`${p.dueDate}T23:59:59`).getTime() < Date.now()
                : false;

              return (
                <div key={p.id} className={`rounded-2xl border bg-surface p-4 space-y-3 ${isProjectOverdue ? 'border-danger/40' : 'border-line'}`}>
                  {/* Header Card */}
                  ...
                  <div className="flex items-center justify-between text-xs text-muted">
                    ...
                    {nearestDeadline && (
                      <div className={`flex items-center gap-1 ${isProjectOverdue ? 'text-danger font-medium' : ''}`}>
                        <Calendar className="h-3.5 w-3.5" />
                        {formatDate(nearestDeadline, i18n.language)}
                        {isProjectOverdue && ` · ${t('task.overdue')}`}
                      </div>
                    )}
                  </div>
